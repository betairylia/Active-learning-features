...
def _weight_drop(module, weights, dropout, apply_getter=None):
    """
    ...
    def forward(*args, **kwargs):
        for name_w in weights:
            ...
            if apply_getter is not None:
                w = torch.nn.functional.dropout(raw_w, p=dropout, training=apply_getter())
            else:
                w = torch.nn.functional.dropout(raw_w, p=dropout, training=module.training)
            
            if name_w in module._parameters:
                del module._parameters[name_w]
...
class WeightDrop(torch.nn.Module):
    ...
    **Thank you** to Sales Force for their initial implementation of :class:`WeightDrop`. Here is
    ...
    def __init__(self, module, weights, dropout=0.0, apply_getter=None):
        super(WeightDrop, self).__init__()
        self.module = module
        _weight_drop(module, weights, dropout, apply_getter)
